...
for DATASET_TYPE in 'mrbrain_t1' #'mrbrain_flair' 'ixi_pd' 'ixi_t2'
    ...
    for MASK_TYPE in 'cartesian' 'gaussian'
	    ...
	    PREDICTIONS_PATH=${EXPERIMENT_PATH}'/'${MODEL}'/results_valid/'${DATASET_TYPE}'/'${MASK_TYPE}'/acc_'${ACC_FACTOR}
            REPORT_PATH=${BASE_PATH}'/experiments/'${MODEL}'/reports_for_seen/'
            echo python valid_measures_csv.py --target-path ${DATA_PATH} --acceleration_factor ${ACC_FACTOR} --dataset_type ${DATASET_TYPE} --mask_type ${MASK_TYPE} --report-path ${REPORT_PATH}
