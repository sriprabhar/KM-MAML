...
from dataset import SliceData,taskdataset
...
def test_time_inference(args,model,test_task_loader,test_support_loader,test_query_loader,writer,km_model,task_encoder):
    ...
    for iter,test_task_mini_batch in enumerate(test_task_loader):
        ...
        for test_task,_ in zip(test_task_mini_batch[0],test_task_mini_batch[1]):
            #test_task_id = test_task_id.unsqueeze(0).to(args.device)
            #task_modulation_weights = km_model(test_task_id) 
            ...
            test_task_id = torch.zeros(256).to(args.device)
            adapt_flag = False
            no_of_gd_steps = 0
            ...
            while True:
                ...
                for gd_steps,test_spt_data_batch in enumerate(test_support_loader[test_task]):
                    ...
                    _,test_task_id = task_encoder(test_spt_us_imgs)
                    test_task_id = test_task_id.unsqueeze(0)
 
                    task_down_modulation_weights, task_latent_modulation_weights,task_up_modulation_weights = km_model.adaptation(test_task_id,km_clone_weights)
                    #task_modulation_weights = km_model.adaptation(test_task_id,km_clone_weights)

                    task_meta_initialization_weights = model.modulation(clone_weights,task_down_modulation_weights, task_latent_modulation_weights,task_up_modulation_weights)
                    #task_meta_initialization_weights = model.modulation(clone_weights,task_modulation_weights)
                    test_spt_fs_pred = model.adaptation(test_spt_us_imgs,task_meta_initialization_weights,test_spt_ksp_imgs,test_spt_ksp_mask)
                    ...
                    if no_of_gd_steps == 1:
                        ...
                        best_adapt_km_model_weights = [w.clone() for w in km_clone_weights]
                    
                    clone_grads = torch.autograd.grad(test_spt_loss, clone_weights,retain_graph=True)
                    clone_weights = [w-args.test_adapt_lr*g for w, g in zip(clone_weights,clone_grads)]

                    #torchviz.make_dot(test_spt_loss, params=dict(model.named_parameters())).render("graph_test", format="png")
                    km_clone_grads = torch.autograd.grad(test_spt_loss, km_clone_weights)
                    km_clone_weights = [w-args.test_adapt_lr*g for w, g in zip(km_clone_weights,km_clone_grads)]

                    if best_adapt_loss > test_spt_loss.item():
                        best_adapt_loss = test_spt_loss.item()
                        best_adapt_model_weights = [w.clone() for w in clone_weights]
                        best_adapt_km_model_weights = [w.clone() for w in km_clone_weights]
            ...
            with torch.no_grad(): # this is added to ensure that gradients do not occupy the gpu mem
                #qry_task_modulation_weights = km_model.adaptation(test_task_id, best_adapt_km_model_weights)
                qry_task_down_modulation_weights, qry_task_latent_modulation_weights,qry_task_up_modulation_weights = km_model.adaptation(test_task_id, best_adapt_km_model_weights)
                #task_meta_initialization_weights = model.modulation(best_adapt_model_weights,qry_task_modulation_weights)
                task_meta_initialization_weights = model.modulation(best_adapt_model_weights,qry_task_down_modulation_weights, qry_task_latent_modulation_weights,qry_task_up_modulation_weights) 
                test_qry_fs_pred = model.adaptation(test_qry_us_imgs,task_meta_initialization_weights,test_qry_ksp_imgs,test_qry_ksp_mask)
...
def main(args):
    ...
    model,km_model,task_encoder = load_model(args.checkpoint)
    
    writer = SummaryWriter(log_dir=str(args.tensorboard_summary_dir / args.summary_folder_name))
    
    reconstructions,_ = test_time_inference(args,model,test_task_loader,test_support_loader,test_query_loader,writer,km_model, task_encoder)
...
#     parser.add_argument('--test_query_batch_size', default=60,type=int, help='Query batch size')
    ...
    parser.add_argument('--summary_folder_name',type=str,help='All the test tasks')
