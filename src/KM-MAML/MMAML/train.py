...
def train_epoch(args,epoch,model,train_task_loader,train_support_dataset,train_query_dataset,train_support_task_index,train_query_task_index,meta_optimizer,writer,km_model,km_optimizer,task_encoder):
    ...
    for iter,train_task_mini_batch in enumerate(train_task_loader):
        ...
        for train_task,_ in zip(train_task_mini_batch[0],train_task_mini_batch[1]):
            ...
            #print(train_task,train_task_id.shape)
            #task_down_modulation_weights, task_latent_modulation_weights,task_up_modulation_weights = km_model(train_task_id) 
            ...
            #print("Use updated: ", train_task_id)
            task_down_modulation_weights, task_latent_modulation_weights,task_up_modulation_weights = km_model(train_task_id) 
            task_meta_initialization_weights = model.modulation(clone_weights,task_down_modulation_weights, task_latent_modulation_weights,task_up_modulation_weights)
        ...
        km_meta_grads = torch.autograd.grad(train_meta_loss/len(train_task_mini_batch),km_weights)
...
def validate(args,epoch,model,val_task_loader,val_support_dataset,val_query_dataset,val_support_task_index,val_query_task_index,writer,km_model,task_encoder):
    ...
    for iter,val_task_mini_batch in enumerate(val_task_loader):
        ...
        for val_task,_ in zip(val_task_mini_batch[0],val_task_mini_batch[1]):
            ...
            #task_down_modulation_weights, task_latent_modulation_weights,task_up_modulation_weights = km_model(val_task_id) 
            ...
            with torch.no_grad(): # this is added to ensure that gradients do not occupy the gpu mem
                qry_task_down_modulation_weights, qry_task_latent_modulation_weights,qry_task_up_modulation_weights = km_model(val_task_id) 
                task_meta_initialization_weights = model.modulation(clone_weights,qry_task_down_modulation_weights, qry_task_latent_modulation_weights,qry_task_up_modulation_weights)
...
def visualize(args, epoch, model, data_loader, writer,task_string,km_model,task_encoder):
    ...
    with torch.no_grad():
        ...
        #task_id = task_id.unsqueeze(0).to(args.device)
        ...
        for iter, data_batch in enumerate(tqdm(data_loader)):
            ...
            #print("us_imgs shape: ", us_imgs.shape)
...
def build_model(args):
    ...
    #basemodel = DC_CNN_MAML(args).to(args.device).double() # double to make the weights in double since input type is double
...
def main(args):
    ...
    display_task_string = task_strings[20]
