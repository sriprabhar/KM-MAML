...
BASE_REPORT_PATH=${EXPERIMENT_PATH}'/'${MODEL}'/results_unseen_adapt_km_only_30'
...
for DATASET_TYPE in 'sri24_t1_few' 'sri24_t2_few' 'sri24_pd_few'
    ...
    for MASK_TYPE in 'cartesian' 'gaussian'
        ...
        for ACC_FACTOR in '4x' '5x' '6x' '7x' '8x' '9x'
            ...
            REPORT_PATH=${BASE_REPORT_PATH}'/report_'${DATASET_TYPE}'_'${MASK_TYPE}'_'${ACC_FACTOR}'_adaptkmonly.txt'
            echo ${REPORT_PATH}
            cat ${REPORT_PATH}
            echo "\n"
            done 
