...
#from models import DnCn, UnetModel
...
def create_datasets(args):
    ...
    display2_data = SliceDisplayDataDev(args.validation_path,'kneeMRI320x320','cartesian','4x',args.usmask_path)
...
def train_epoch(args, epoch, model,data_loader, optimizer, writer,km_model):
    ...
    for iter, data in enumerate(tqdm(data_loader)):
        ...
        us_input, input_kspace, target, gamma_val, acc_factor_string, mask_string, dataset_string,mask = data
        ...
        gamma_val = gamma_val.unsqueeze(1).to(args.device)
...
#        output = model(us_input,input_kspace,mask)
        ...
        output = model(us_input,input_kspace, acc_factor_string, mask_string, args.dataset_type,mask,down_modulation_weights, latent_modulation_weights,up_modulation_weights)
...
def evaluate(args, epoch, model, data_loader, writer,km_model):
...
def visualize(args, epoch, model, data_loader, writer,datasettype_string,km_model):
...
def save_model(args, exp_dir, epoch, model, optimizer,best_dev_loss,is_new_best,km_model):
...
def build_model(args):
    ...
    basemodel = DC_CNN_MAML(args).to(args.device)
...
def main(args):
    ...
    if args.resume:
        ...
        model,km_model = build_model(args)
        #print ("Model Built")
        if args.data_parallel:
            model = torch.nn.DataParallel(model)    
        #optimizer = build_optim(args, model.parameters())
        optimizer = build_optim(args, list(model.parameters()) + list(km_model.parameters()))
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        train_loss,train_time = train_epoch(args, epoch, model, train_loader,optimizer,writer,km_model)
        dev_loss,dev_time = evaluate(args, epoch, model, dev_loader, writer,km_model)
        visualize(args, epoch, model, display1_loader, writer,'t1',km_model)
        visualize(args, epoch, model, display2_loader, writer,'knee',km_model)

        is_new_best = dev_loss < best_dev_loss
        best_dev_loss = min(best_dev_loss,dev_loss)
        save_model(args, args.exp_dir, epoch, model, optimizer,best_dev_loss,is_new_best,km_model)
        logging.info(
